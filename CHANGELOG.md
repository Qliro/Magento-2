...
- The admin and the customer order view show the payment method name Qliro sent instead of the raw method code, falling back to the code when no name was stored. The admin keeps the code in its own row for support, and only when it differs from the name. The printed documents get the same name, see the entry above. Qliro keeps adding method codes, the six `QLIROPAYLATER_*` ones from the Ironman rollout among them, and `QLIROPAYLATER_INVOICE30` is not something a merchant can read (PLIN-374)
