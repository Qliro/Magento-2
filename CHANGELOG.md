...
- The invoice, the credit memo and the shipment print the named payment method, the Qliro order id and the Qliro reference. That template printed a hardcoded `Identification Number: 123` and nothing else, on every Qliro order of every merchant, which is the one document the customer keeps. Magento draws this block as plain lines rather than as the table the order view uses, so it is written that way, and a value the order does not carry is left out instead of printing an empty label (PLIN-374)
- The order view names the payment method: `Invoice, 30 days` rather than `QLIROPAYLATER_INVOICE30`, and `Qliro invoice` rather than `INVOICE`. Qliro states the method twice and neither field is meant for a person: `PaymentMethod.PaymentMethodName` is the product, which is what the Ironman rollout renames and where the six `QLIROPAYLATER_*` values arrive, and `PaymentMethod.PaymentTypeCode` is the instrument behind it, which reads `INVOICE` for every pay later product old and new alike and on live orders can be `MASTERCARD` or a bare number such as `704`. The order view printed the type code, so it could not tell an Ironman order from a legacy one at all. The wording is in `Model/PaymentMethodLabel.php`, translated like any other string, and covers the six new products, the ones they migrate from, and the other methods Qliro can route an order to (PLIN-374)
- A method the table has no wording for is printed as Qliro named it rather than hidden, so a product launched after this release reads as `QLIROPAYLATER_SOMETHING_NEW` instead of as an empty cell. An order stored before the module recorded the name resolves its wording from the type code instead (PLIN-374)
- The admin order view states the two raw values under the wording, `Qliro Payment Method` and `Payment Type Code`, each only where it says something the wording does not. Support reads by those, the customer sees only the wording (PLIN-374)
