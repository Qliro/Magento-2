...
- The cart is refused before it becomes an order, at the Qliro checkout page and again when the order is placed, with a message naming the line the buyer has to change. A store selling by weight or length, or using a fractional `qty_increments`, is what produces such a cart. Truncating it made the line total wrong, and Qliro refuses the whole order on a totals mismatch halfway through the checkout, which is what a buyer saw instead (PLIN-367)
- The validate callback declines a cart that turned fractional after the Qliro order was created, naming the line in the log. Otherwise the order would be placed for a quantity Qliro was never told about (PLIN-367)
