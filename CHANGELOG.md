...
- The shipping line and the invoice fee line of a capture state their VAT rate. `ShippingFeeHandler` and `InvoiceFeeHandler` set the two price fields and never called `setVatRate()`, so both lines left with the `Item` default of 0 while the product and discount lines in the same payload carried a rate. Qliro's invoice then stated no VAT on the shipping and the fee, and a finance user reconciling a capture against the Magento order found VAT missing on exactly those two rows. The capture payload now describes its own lines: the rate is derived from the amounts the line carries, taken before they are rounded for sending, and a line that genuinely carries no VAT still states 0. Reading it off the rounded figures instead would state a rate no jurisdiction charges: a shipping price of 4.79 taxed at 25 percent is 5.9875, goes out as 5.99, and 5.99 over 4.79 reads back as 25.05. Whether the invoice a finance user reads shows that rate is not settled here: review traced the payments service dropping `VatRate` from `MarkItemsAsShipped` before validation, which would mean the invoice takes its VAT from the reservation and the stored order item rate instead. The missing VAT report is tracked on its own, and this release is not being claimed as the fix for it (PLIN-361)
